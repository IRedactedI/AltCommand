...
local function withStyleVars(vars, func)
...
local function renderWindow(window, windowName, isPreview)
    ...
    withStyleVars(
        ...
        function()
            ...
            local totalHeight =
                (effectiveButtonHeight * totalRows) + (totalRows - 1) * buttonSpacing +
            ...
            if imgui.Begin(windowName, true, windowFlags) then
                ...
                for i = 1, #commands, maxButtonsPerRow do
                    ...
                    local totalButtonWidth =
                        (buttonsInRow * effectiveButtonWidth) + ((buttonsInRow - 1) * buttonSpacing)
                    ...
                    for j = 0, buttonsInRow - 1 do
                        ...
                        if buttonClicked then
                            ...
                            elseif command.commandType == "isWindow" then
                                ...
                                local targetWindow =
                                    altCommand.settings.windows[command.windowToggleName or command.text]
                            ...
                            if newWindowDialog.windowName[1] == "" then
                                print(
                                    chat.header(addon.name):append(
                                        chat.error("Please enter a name for the new window.")
                                    )
                                )
                            else
                            ...
                            if selectedWindow then
                                ...
                                if selectedWindow.type == "imgbutton" then
                                    ...
                                    if imgui.InputText("##texturePath", addButtonDialog.texturePath, 256) then
                                        local texturePath =
                                            addon.path .. "/resources/" .. addButtonDialog.texturePath[1]
                                ...
                                if imgui.Button("Add Button##ConfirmAdd") then
                                    if addButtonDialog.commandName[1] == "" then
                                        ...
                                        print(
                                            chat.header(addon.name):append(
                                                chat.message(
                                                    "New button added to window '" ..
                                                        addButtonDialog.selectedWindow .. "'."
                                                )
                                            )
                                        )
                                    end
